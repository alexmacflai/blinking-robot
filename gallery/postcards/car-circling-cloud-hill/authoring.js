...
r(car, 'full-car scale', 'car.scale', 0.5, 2, 0.01, v => v.toFixed(2));
r(car, 'size variation', 'car.sizeVary', 0, 0.6, 0.02, v => v.toFixed(2));
r(car, 'body length min', 'car.bodyLengthMin', 0.7, 1.2, 0.01, v => v.toFixed(2));
r(car, 'body length max', 'car.bodyLengthMax', 0.8, 1.3, 0.01, v => v.toFixed(2));
r(car, 'width', 'car.widthRatio', 0.25, 1, 0.01, v => v.toFixed(2));
r(car, 'height', 'car.heightRatio', 0.2, 1, 0.01, v => v.toFixed(2));
r(car, 'body height min', 'car.bodyHeightMin', 0.7, 1.2, 0.01, v => v.toFixed(2));
r(car, 'body height max', 'car.bodyHeightMax', 0.8, 1.3, 0.01, v => v.toFixed(2));
r(car, 'cabin length min', 'car.cabinLengthMin', 0.2, 0.45, 0.01, v => v.toFixed(2));
r(car, 'cabin length max', 'car.cabinLengthMax', 0.25, 0.6, 0.01, v => v.toFixed(2));
r(car, 'cabin height min', 'car.cabinHeightMin', 0.35, 0.85, 0.01, v => v.toFixed(2));
r(car, 'cabin height max', 'car.cabinHeightMax', 0.45, 1, 0.01, v => v.toFixed(2));
r(car, 'vertical bob amplitude', 'car.bobAmplitude', 0, 1, 0.01, v => v.toFixed(2));
r(car, 'vertical bob speed', 'car.bobSpeed', 0, 3, 0.01, v => v.toFixed(2));
r(car, 'lateral sway amplitude', 'car.swayAmplitude', 0, 1, 0.01, v => v.toFixed(2));
r(car, 'lateral sway speed', 'car.swaySpeed', 0, 1, 0.01, v => v.toFixed(2));
...
r(car, 'front taper', 'car.bodyFrontRatio', 0.45, 1, 0.01, v => v.toFixed(2));
r(car, 'roof inset', 'car.bodyTopRatio', 0.6, 1, 0.01, v => v.toFixed(2));
r(car, 'cabin roof width', 'car.cabinTopRatio', 0.55, 1, 0.01, v => v.toFixed(2));
r(car, 'windscreen slope', 'car.cabinFrontSlope', 0.35, 1, 0.01, v => v.toFixed(2));
r(car, 'wheel radius', 'car.wheelRadius', 0.12, 0.36, 0.01, v => v.toFixed(2));
r(car, 'lamp radius', 'car.lampRadius', 0.03, 0.16, 0.005, v => v.toFixed(3));
r(car, 'lamp tone', 'car.lampTone', 0, 1, 0.005, v => v.toFixed(3));
r(car, 'headlight strength', 'car.headlightStrength', 0, 1, 0.01, v => v.toFixed(2));
r(car, 'headlight tone', 'car.headlightTone', 0, 1, 0.01, v => v.toFixed(2));
r(car, 'headlight reach', 'car.headlightReach', 0.05, 1, 0.01, v => v.toFixed(2));
car.note('Construction controls shape the car and its attached details. Wheel centres move with the current body footprint, while wheel and lamp radii stay fixed during body/cabin variation. The headlights are projected from the attached lamps onto the first road/hill surface.');
